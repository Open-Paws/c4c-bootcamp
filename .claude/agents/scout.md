---
description: QA agent — exercises repo content as real users, files structured GitHub issues for failures
---
...
2. Set up the environment if required (install dependencies as described in CLAUDE.md or README):
   ```bash
...
4. After each flow, check all `assertions`:
   - `succeeds` — these must complete without error (exit code 0)
...
## Command / call
<exact command or call that failed>
